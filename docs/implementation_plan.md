...
- `drawFloor()` で現在階のみ角度差を参照し、穴が近い時と通過可能範囲で発光量を変える補助表示を追加した。
- `update(dt)` で危険床接触時に `pendingFinish` を設定し、約0.35秒は更新・スコア加算・ランキング送信を止めて失敗箇所を見せる流れにした。
- `drawPlayFeedback()` でコンボ表示、一気落下予告、危険床失敗位置の表示をCanvasに集約し、DOM更新を増やさない構成にした。
- `resultComment()` と結果画面の `resultComment` 要素で、終了理由や到達階に応じた評価コメントを表示する。
- ランキング送信・取得失敗時は画面文言を短くし、詳細は `console.warn` のみに残す。
- README、仕様書、レビュー確認リストに、今回追加したユーザー向け改善と確認観点を反映した。

## 結果コメント差し替え 実装計画

- 現行 `index.html` の `resultComment()` と `finish()` の呼び出しを確認したうえで、結果コメントを終了理由優先の固定文から到達層ベースの対応表へ差し替える。
- `resultComment(won, reason, reachedFloor)` として到達層を明示的に受け取り、リタイアや危険床接触でも到達層に応じたコメントを返す。
- `finish()` では結果画面へ表示する到達階と同じ値を `reachedFloor` として渡し、終了理由欄は従来どおり表示する。
- 31〜35層、36〜39層、40層クリアの文言も防御的に実装するが、この変更だけでクリア条件や最大階層を変更しない。
- ランキング送信・ランキング取得、Supabase設定、`submit_score`、`get_best_score_ranking`、スコア計算、左右ドラッグ操作、Canvas描画、危険床失敗演出、850msコンボ処理、`pendingFinish` 中の入力無効化には影響させない。
