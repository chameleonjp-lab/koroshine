...
- `resultComment()` と結果画面の `resultComment` 要素で、到達層だけに応じた評価コメントを表示する。終了理由は別欄で維持する。
- ランキング送信・取得失敗時は画面文言を短くし、詳細は `console.warn` のみに残す。
- README、仕様書、レビュー確認リストに、今回追加したユーザー向け改善と確認観点を反映した。

## 結果コメント差し替え 実装計画

- 40層仕様（`MAX_FLOOR = 40`、40階到達クリア、40階到達時 `+3000`）は維持する。
- 現行 `index.html` の `resultComment()` と `finish()` の呼び出しを確認したうえで、結果コメントを終了理由優先の固定文から到達層ベースの対応表へ差し替える。
- `resultComment()` から `won` / `reason` 優先分岐を削除し、到達層だけで結果コメントを決定する。
- `finish()` では結果画面へ表示する到達階と同じ値を `reachedFloor` として渡し、終了理由欄は従来どおり表示する。
- ランキング送信・ランキング取得、Supabase設定、`submit_score`、`get_best_score_ranking`、スコア計算、ゲーム本体、左右ドラッグ操作、Canvas描画、危険床失敗演出、850msコンボ処理、`pendingFinish` 中の入力無効化には影響させない。
